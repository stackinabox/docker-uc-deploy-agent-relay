...
####
ARTIFACT_URL=${ARTIFACT_URL:-http://artifacts.stackinabox.io/urbancode/ibm-ucd-agent-relay}
...
ARTIFACT_DOWNLOAD_URL=${ARTIFACT_DOWNLOAD_URL:-$ARTIFACT_URL/$ARTIFACT_VERSION/agent-relay-$ARTIFACT_VERSION.zip}
...
docker build --tag="stackinabox/urbancode-deploy-agent-relay:$ARTIFACT_STREAM" \
             --tag="stackinabox/urbancode-deploy-agent-relay:$ARTIFACT_VERSION" \
